...
class ConvolutionalVAE(nn.Module):
    def __init__(self, 
        ...
        if self.decoder_type == 'upsample_conv':
            ...
            logger.info(f"  Target spatial dims (from encoder, reversed an prepended with input_size for full path): {self.encoder_spatial_dims[-2::-1] + [self.encoder_spatial_dims[0]]}")


            for i in range(self.num_conv_layers_encoder): # Iterates N times for N conv_transpose layers
                k = decoder_kernels[i]
                s = decoder_strides[i] 
                p = decoder_paddings[i]
                
                # Target output dimension for THIS ConvTranspose layer is the spatial dimension of the
                # corresponding encoder stage (before the downsampling Conv of that stage)
                # encoder_spatial_dims = [H_in, H_enc1_out, H_enc2_out, ..., H_enc_LAST_out]
                # For decoder layer i (0-indexed, N layers total):
                #   - Target output should match encoder_spatial_dims[N - 1 - i]
                #   - Or, equivalently, encoder_spatial_dims[-(i+2)]
