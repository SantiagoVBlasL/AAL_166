...
from typing import Optional, List, Dict, Tuple # Asegurar que Tuple está importado
...
class ConvolutionalVAE(nn.Module):
    def __init__(self, input_channels=6, latent_dim=128, image_size=131, final_activation='sigmoid'):
        ...
        self.encoder_conv = nn.Sequential(
            nn.Conv2d(input_channels, 32, kernel_size=7, stride=2, padding=3), # -> (B, 32, H/2, W/2)
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.Dropout2d(p=0.2), 
        ...
        with torch.no_grad():
            ...
            self.final_spatial_dim_encoder = self.encoder_conv(dummy_input).shape[-1] # Renombrado para claridad
            self.flattened_size = 256 * self.final_spatial_dim_encoder * self.final_spatial_dim_encoder
            logger.info(f"VAE Encoder: Calculated flattened size = {self.flattened_size} (final spatial dim after encoder: {self.final_spatial_dim_encoder})")
        ...
        decoder_layers = [
            ...
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.ConvTranspose2d(32, input_channels, kernel_size=7, stride=2, padding=3, output_padding=1)
        ]
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        h = self.decoder_fc(z)
        h = h.view(h.size(0), 256, self.final_spatial_dim_encoder, self.final_spatial_dim_encoder) # Unflatten
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if recon_x.shape != x.shape:
             ...
             logger.warning(f"Shape mismatch even after crop/pad: Input {x.shape}, Recon {recon_x.shape}. Attempting resize.")
...
def normalize_inter_channel_fold(
    ...
    """
    logger.info(f"Aplicando normalización inter-canal (modo: {mode}) usando parámetros de entrenamiento.")
    ...
    for c_idx in range(num_channels):
        ...
        for subj_idx in range(channel_data_train.shape[0]):
            all_off_diag_train_values.extend(channel_data_train[subj_idx][off_diag_mask])
        all_off_diag_train_values = np.array(all_off_diag_train_values)

        params = {}
        if all_off_diag_train_values.size == 0:
            logger.warning(f"Canal {c_idx}: No hay elementos fuera de la diagonal en el conjunto de entrenamiento. No se escala.")
            ...
            params = {'mean': mean_val, 'std': std_val if std_val > 1e-9 else 1.0, 'mode': mode}
            if std_val <= 1e-9:
                logger.warning(f"Canal {c_idx}: STD muy bajo en entrenamiento ({std_val:.2e}). Se usará STD=1 para evitar división por cero.")
            ...
            if (max_val - min_val) <= 1e-9:
                 logger.warning(f"Canal {c_idx}: Rango (max-min) muy bajo en entrenamiento ({(max_val - min_val):.2e}). La escala MinMax puede ser inestable.")
        else:
            ...
            if off_diag_mask.any():
                if mode == 'zscore_offdiag':
                    ...
                    if (params.get('max', 1.0) - params.get('min', 0.0)) > 1e-9: 
                        scaled_matrix_ch[off_diag_mask] = (current_matrix[off_diag_mask] - params['min']) / (params['max'] - params['min'])
                    else: 
                        scaled_matrix_ch[off_diag_mask] = 0.0 
            normalized_tensor_fold[subj_glob_idx, c_idx, :, :] = scaled_matrix_ch
        
        if mode == 'zscore_offdiag':
            logger.info(f"Canal {c_idx}: Off-diagonal Z-score aplicado (train_mean={params['mean']:.3f}, train_std={params['std']:.3f}).")
        elif mode == 'minmax_offdiag':
            logger.info(f"Canal {c_idx}: Off-diagonal MinMax escalado aplicado (train_min={params['min']:.3f}, train_max={params['max']:.3f}).")
            
    return normalized_tensor_fold, norm_params_per_channel

def apply_normalization_params(data_tensor_subset: np.ndarray, norm_params_per_channel: List[Dict[str, float]]) -> np.ndarray:
...
def train_and_evaluate_pipeline(global_tensor: np.ndarray, metadata_df: pd.DataFrame, args: argparse.Namespace):
    """
    ...
    for fold_idx, (train_dev_clf_indices_in_cn_ad, test_clf_indices_in_cn_ad) in enumerate(outer_cv_clf.split(np.zeros(len(y_classifier)), y_classifier)):
        logger.info(f"--- Iniciando Fold Externo {fold_idx + 1}/{args.outer_folds} ---")

        global_train_dev_clf_subject_indices = X_classifier_indices[train_dev_clf_indices_in_cn_ad]
        global_test_clf_subject_indices = X_classifier_indices[test_clf_indices_in_cn_ad]
        
        all_subject_global_indices = metadata_df['tensor_idx'].values
        vae_train_dev_global_indices = np.setdiff1d(all_subject_global_indices, global_test_clf_subject_indices, assume_unique=True)

        vae_train_dev_tensor = global_tensor[vae_train_dev_global_indices]
        
        logger.info("Normalizando datos para VAE (entrenamiento VAE y test del clasificador)...")
        ...
        test_clf_tensor_norm = apply_normalization_params(test_clf_tensor_subset_global, norm_params_fold)


        vae_dataset = TensorDataset(torch.from_numpy(vae_train_dev_tensor_norm).float())
        vae_dataloader = DataLoader(vae_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True)

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        logger.info(f"Usando dispositivo: {device}")
        
        logger.info(f"VAE decoder final activation set to: {args.vae_final_activation} based on norm_mode: {args.norm_mode}")
        ...
        for epoch in range(args.epochs_vae):
            ...
            if (epoch + 1) % 10 == 0: 
                logger.info(f"Fold {fold_idx+1}, VAE Epoch {epoch+1}/{args.epochs_vae}, Loss: {avg_epoch_loss:.4f}")
        
        vae_model_path = Path(args.output_dir) / f"vae_fold_{fold_idx+1}_ld{args.latent_dim}_beta{args.beta_vae}.pt"
        torch.save(vae.state_dict(), vae_model_path)
        logger.info(f"Modelo VAE del fold {fold_idx+1} guardado en: {vae_model_path}")
        
        train_dev_clf_tensor_subset_global = global_tensor[global_train_dev_clf_subject_indices]
        train_dev_clf_tensor_norm = apply_normalization_params(train_dev_clf_tensor_subset_global, norm_params_fold)
        
        vae.eval()
        with torch.no_grad():
            mu_train_clf, _ = vae.encode(torch.from_numpy(train_dev_clf_tensor_norm).float().to(device))
            mu_test_clf, _ = vae.encode(torch.from_numpy(test_clf_tensor_norm).float().to(device)) 
        ...
        if X_train_latent.shape[0] == 0 or X_test_latent.shape[0] == 0:
            logger.warning(f"Fold {fold_idx+1}: No data for classifier training or testing after filtering CN/AD. Skipping.")
            fold_metrics.append({'fold': fold_idx + 1, 'auc': np.nan, 'pr_auc': np.nan, 'accuracy': np.nan, 'sensitivity': np.nan, 'specificity': np.nan, 'f1_score': np.nan})
            continue
        ...
        param_grid_svm = {'C': [0.1, 1, 10, 100], 'gamma': [0.001, 0.01, 0.1, 1, 'scale', 'auto'], 'kernel': ['rbf']}
        ...
        svm_clf = GridSearchCV(SVC(probability=True, random_state=args.seed, class_weight='balanced' if args.class_weight_svm else None), 
                               param_grid_svm, cv=inner_cv_svm, scoring='roc_auc', n_jobs=1)
        ...
        logger.info(f"Fold {fold_idx + 1} - Resultados del Clasificador en Test Externo:")
        logger.info(f"  AUC: {auc:.4f}, PR-AUC: {pr_auc:.4f}, Accuracy: {acc:.4f}")
        logger.info(f"  Sensitivity (AD): {sens:.4f}, Specificity (CN): {spec:.4f}, F1-score (AD): {f1:.4f}")
        
        train_ids_str = ",".join(sorted(cn_ad_df.iloc[train_dev_clf_indices_in_cn_ad]['SubjectID'].tolist()))
        test_ids_str = ",".join(sorted(cn_ad_df.iloc[test_clf_indices_in_cn_ad]['SubjectID'].tolist()))
        logger.info(f"  Fold {fold_idx+1} Train IDs Hash: {hashlib.md5(train_ids_str.encode()).hexdigest()}")
        logger.info(f"  Fold {fold_idx+1} Test IDs Hash: {hashlib.md5(test_ids_str.encode()).hexdigest()}")

        fold_metrics.append({'fold': fold_idx + 1, 'auc': auc, 'pr_auc': pr_auc, 'accuracy': acc, 'sensitivity': sens, 'specificity': spec, 'f1_score': f1})
    ...
    if fold_metrics:
        ...
        logger.info(f"AUC:         {metrics_df['auc'].mean():.4f} +/- {metrics_df['auc'].std():.4f}")
        logger.info(f"PR-AUC:      {metrics_df['pr_auc'].mean():.4f} +/- {metrics_df['pr_auc'].std():.4f}")
        logger.info(f"Accuracy:    {metrics_df['accuracy'].mean():.4f} +/- {metrics_df['accuracy'].std():.4f}")
        logger.info(f"Sensitivity: {metrics_df['sensitivity'].mean():.4f} +/- {metrics_df['sensitivity'].std():.4f}")
        logger.info(f"Specificity: {metrics_df['specificity'].mean():.4f} +/- {metrics_df['specificity'].std():.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global_tensor_path", type=str, required=True, help="Ruta al archivo .npz del tensor global de conectividad.")
    parser.add_argument("--metadata_path", type=str, required=True, help="Ruta al archivo CSV de metadatos de sujetos (ej. SubjectsData_Schaefer2018_400ROIs.csv).")
    parser.add_argument("--output_dir", type=str, default="./vae_classification_output", help="Directorio para guardar resultados y modelos VAE por fold.")
    
    # Parámetros de CV
    parser.add_argument("--outer_folds", type=int, default=5, help="Número de folds para la CV externa.")
    parser.add_argument("--inner_folds", type=int, default=3, help="Número de folds para la CV interna (optimización de hiperparámetros del clasificador).")
    
    # Parámetros del VAE
    parser.add_argument("--latent_dim", type=int, default=64, help="Dimensionalidad del espacio latente del VAE.") 
    parser.add_argument("--lr_vae", type=float, default=1e-4, help="Tasa de aprendizaje para el VAE.")
    parser.add_argument("--epochs_vae", type=int, default=50, help="Número de épocas para entrenar el VAE.") 
    parser.add_argument("--batch_size", type=int, default=16, help="Tamaño del batch para el VAE.") 
    parser.add_argument("--beta_vae", type=float, default=1.0, help="Factor beta para la pérdida KL en el VAE.")
    parser.add_argument("--weight_decay_vae", type=float, default=1e-5, help="Weight decay para el optimizador Adam del VAE.")
    ...
    parser.add_argument("--class_weight_svm", action='store_true', help="Usar class_weight='balanced' en SVM.")
    ...
    if global_tensor_data is not None and metadata_df_full is not None:
        ...
        logger.info(f"Activación final del decoder VAE seleccionada: {args.vae_final_activation}")
        if args.norm_mode == "minmax_offdiag" and args.vae_final_activation != "sigmoid":
            logger.warning("Se seleccionó norm_mode='minmax_offdiag' pero la activación final del VAE no es 'sigmoid'. "
                           "Esto podría llevar a un rango de reconstrucción inconsistente.")
        if args.norm_mode == "zscore_offdiag" and args.vae_final_activation == "sigmoid":
            logger.warning("Se seleccionó norm_mode='zscore_offdiag' pero la activación final del VAE es 'sigmoid'. "
        ...
        logger.critical("No se pudieron cargar los datos. Abortando el pipeline de entrenamiento.")
